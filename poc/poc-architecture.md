...
> **Registry** knows *what exists* (start here) · the **reuse-detection service** *verifies* what is
...
classification — lives **once** in the shared comparison core and is reused across scopes. Only the
*candidate source* and the *governance metadata* differ:
...
- `DRY_MANIFESTS_DIR` (default `poc/scenarios/scenario-2/registry-manifests`) — where the pure-YAML
  manifests live. The loader walks it recursively for every `kind: DryArtifact` document.
- `DRY_WORKSPACE_DIR` (default `poc/scenarios/scenario-2/workspace`) — the root that each binding's
...
python ../workspace-similarity/scan.py --query ../demo/arpac-authoring-scratch.sql   # Pattern 2
```

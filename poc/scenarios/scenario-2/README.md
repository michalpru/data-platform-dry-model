...
The governed artifacts this scenario resolves against live in the mocked registry at
[`../../registry/manifests/`](../../registry/manifests/) (registered manifests under
`registered/`, with their source manifests under `domains/` and `platform/`). There is no separate
`workspace/` folder here because the "workspace" *is* the registry — that is the point of the scenario.
